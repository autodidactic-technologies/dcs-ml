...
ACTION_LOOKUP = {
    ...
    1: SHOOT_GOAL,
    2: SHOOT_GOAL,
    3: DRIBBLE,  # newly exposed dribble action
}

# field bounds seem to be 0, PITCH_LENGTH / 2, -PITCH_WIDTH / 2, PITCH_WIDTH / 2
PARAMETERS_MIN = [
    np.array([0, -PITCH_WIDTH / 2]),  # KICK_TO (2D)
    np.array([-GOAL_WIDTH / 2]),      # SHOOT_GOAL left (1D)
    np.array([-GOAL_WIDTH / 2]),      # SHOOT_GOAL right (1D)
    np.array([0, -PITCH_WIDTH / 2]),  # DRIBBLE (2D)
]

PARAMETERS_MAX = [
    np.array([PITCH_LENGTH, PITCH_WIDTH / 2]),  # KICK_TO
    np.array([GOAL_WIDTH / 2]),                 # SHOOT_GOAL left
    np.array([GOAL_WIDTH / 2]),                 # SHOOT_GOAL right
    np.array([PITCH_LENGTH, PITCH_WIDTH / 2]),  # DRIBBLE
]
