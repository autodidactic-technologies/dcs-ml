    ...
    def agent_run(self, sim_step, obs, action, infos, env=None):
        ...
        try:
            ...
            if self.verbose:
                ...
                if overridden:
                    logger.info(f"[LLM @ Step {sim_step},{front_obj}] OVERRIDDEN by LLM: PPO {action} → LLM {llm_action}, EXPLANATION: {explanation}")
                else:
                    logger.info(f"[LLM @ Step {sim_step},{front_obj}] LLM agrees with PPO: {action}, EXPLANATION: {explanation}")
            print()
